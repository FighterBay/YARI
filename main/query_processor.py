...
async def answer_query(query_dict: dict, doc_version: int):
    """
    ...
    Args:
        ...
        version (int): Version of the ingested document
    Returns:
        str: The response generated by OpenAI based on the query and retrieved context.
    """
    try:
        utils.logger.info("doc_version: %d", doc_version)
        config = utils.get_config()
        file_hash, query = list(query_dict.items())[0]

        redis_cache = redis_utils.RedisQueryCache(file_hash)

        if doc_version > 0:
            cached_answer = redis_cache.get_query_cache(query, doc_version)
            if cached_answer:
                utils.logger.info(
                    "Found cached answer for %s in %s cache", query, file_hash)
        ...
        redis_cache.add_query_cache(query, completion, doc_version)
